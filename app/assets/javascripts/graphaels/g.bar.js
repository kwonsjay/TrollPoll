 ...
 \*/
    ...
    function VBarchart(paper, x, y, width, height, values, opts) {
        ...
        var chartinst = this,
            ...
            len = values.length,
						votes = opts.votes || chartinst.votes,
						cssEls = opts.cssEls || chartinst.cssEls;
        ...
        for (var i = 0; i < len; i++) {
            ...
            for (var j = 0; j < (multi || 1); j++) {
                ...
                bar.value = multi ? values[j][i] : values[i];
								bar.cssEl = cssEls[multi ? j : i];
								bar.vote = votes[multi ? j : i];
 ...
 \*/
    ...
    function HBarchart(paper, x, y, width, height, values, opts) {
        ...
        var chartinst = this,
            ...
            colors = opts.colors || chartinst.colors,
            len = values.length,
						percents = opts.percents || chartinst.percents,
						cssEls = opts.cssEls || chartinst.cssEls;
        ...
        for (var i = 0; i < len; i++) {
            ...
            for (var j = 0; j < (multi || 1); j++) {
                var val = multi ? values[j][i] : values[i],
                    bar = finger(x, Y + barheight / 2, Math.round(val * X), barheight - 1, false, type, null, paper).attr({stroke: "none", cssEl: cssEls[multi ? j : i], percent: percents[multi ? j : i], fill: colors[multi ? j : i]});
                if (multi) {
                    bars[j].push(bar);
                } else {
                    bars.push(bar);
